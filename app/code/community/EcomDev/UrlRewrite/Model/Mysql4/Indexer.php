...
class EcomDev_UrlRewrite_Model_Mysql4_Indexer extends Mage_Index_Model_Mysql4_Abstract
{
    ...
    protected function _getProductUrlKeySelect()
    {
        ...
        $visibilityAttribute = $this->_getEavConfig()->getAttribute(self::ENTITY_PRODUCT, 'visibility');
        ...
        $select
            ...
            ->joinLeft(array('name_store' => $nameAttribute->getBackendTable()), 
                   ...
                   . $this->_quoteInto(
                       ...
                       $nameAttribute->getAttributeId()),
                   array())
            // visibility attribute values retrieval (for default and store)
            ->join(array('visibility_default' => $visibilityAttribute->getBackendTable()),
                   'visibility_default.entity_id = product.entity_id'
                   . ' AND visibility_default.store_id = 0'
                   . $this->_quoteInto(
                       ' AND visibility_default.attribute_id = ?',
                       $visibilityAttribute->getAttributeId()),
                   array())
            ->joinLeft(array('visibility_store' => $visibilityAttribute->getBackendTable()),
                   'visibility_store.entity_id = product.entity_id'
                   . ' AND visibility_store.store_id = store.store_id'
                   . $this->_quoteInto(
                       ' AND visibility_store.attribute_id = ?',
                       $visibilityAttribute->getAttributeId()),
                   array())
            // Url key attribute retrieval (for default and store)
            ->joinLeft(array('url_key_default' => $urlKeyAttribute->getBackendTable()), 
        ...
        $select
            ->columns($columns)
            ->where(
                'IFNULL(visibility_store.value, visibility_default.value) != ? ',
    ...
    /**
     * Remove not visible products from index tables
     *
     * @param null $productIds
     * @return EcomDev_UrlRewrite_Model_Mysql4_Indexer
     */
    public function clearNotVisibleProducts($productIds = null)
    {
        $tables = array(
        ...
        //we trying to remvoe all not visible items from rewrite tables
        if (is_null($productIds)) {
            $visibilityAttribute = $this->_getEavConfig()->getAttribute(self::ENTITY_PRODUCT, 'visibility');

            $select = $this->_select()
                        // visibility attribute values retrieval (for default and store)
                        ->from(array('main' => 'PLACEHOLDER'))
                        ->join(array('visibility_default' => $visibilityAttribute->getBackendTable()),
                        'visibility_default.entity_id = main.product_id'
                            . ' AND visibility_default.store_id = 0'
                            . $this->_quoteInto(
                            ' AND visibility_default.attribute_id = ?',
                            $visibilityAttribute->getAttributeId()),
                        array())
                        ->joinLeft(array('visibility_store' => $visibilityAttribute->getBackendTable()),
                        'visibility_store.entity_id = main.product_id'
                            . ' AND visibility_store.store_id = main.store_id'
                            . $this->_quoteInto(
                            ' AND visibility_store.attribute_id = ?',
                            $visibilityAttribute->getAttributeId()),
                        array())
            ...
            foreach ($tables as $tableName) {
                $this->_getIndexAdapter()->query(
                    str_replace('PLACEHOLDER', $tableName, $select->deleteFromSelect('main'))
                );
            }
        } else {
            //we don't know is product hidden for any website because data have not been committed to DB yet
            //so we'll remove any reference to this products
            foreach ($tables as $tableName) {
                $this->_getIndexAdapter()->delete($tableName, array('product_id IN(?)' => $productIds));
            }
        }
    ...
    protected function _clearInvalidProductRewrites()
    {
        ...
        if (!Mage::getStoreConfig(Mage_Catalog_Helper_Product::XML_PATH_PRODUCT_URL_USE_CATEGORY)) {
            $this->_getWriteAdapter()->delete($this->getTable('core/url_rewrite'), '(category_id IS NOT NULL AND product_id IS NOT NULL)');
        }
...
//        $this->_getIndexAdapter()->commit();
    ...
    public function updateProductRewrites(array $productIds, array $categoryIds = null)
    {
        $this
            ->_generateTransliterateData(true)
            ->clearNotVisibleProducts($productIds)
